...
function CopyButton({ value, label }: { value: string; label: string }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1 font-medium text-[var(--muted)] hover:text-[var(--ink)] transition-colors"
    >
...
export default function AnswerCard({ synthesis, blobId, durationMs, sessionId, createdAt }: AnswerCardProps) {
  ...
  return (
    <div className="anim-fade-up flex gap-3 sm:gap-4">
      {/* avatar */}
      <span className="flex-shrink-0 mt-0.5 w-8 h-8 rounded-full border border-[var(--line)] bg-[var(--card)] flex items-center justify-center">
        <MnemosLogo size={18} />
      </span>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center flex-wrap gap-2 mb-4">
          ...
          <span className="text-[11px] font-medium text-[var(--muted)] bg-[var(--card)] border border-[var(--line)] rounded-full px-2 py-0.5">
          ...
          {durationMs ? <span className="text-[11px] text-[var(--faint)]">{(durationMs / 1000).toFixed(1)}s</span> : null}
        </div>
              ...
              {theme.supporting_findings?.length > 0 && (
                <ul className="space-y-1.5">
                  {theme.supporting_findings.map((f, j) => (
                    <li key={j} className="flex gap-2.5 text-[15px] text-[var(--muted)] leading-relaxed">
                      <span className="mt-[0.6rem] flex-shrink-0 w-1 h-1 rounded-full bg-[var(--faint)]" />
                      {f}
                    </li>
                  ))}
                </ul>
        ...
        {gaps.length > 0 && (
          <div className="mt-6">
            <p className="text-[11px] font-semibold tracking-widest uppercase text-[var(--faint)] mb-2">Open questions</p>
            <ul className="space-y-1.5">
              {gaps.map((gap, i) => (
        ...
        {contradictions.length > 0 && (
          <div className="mt-6">
            <p className="text-[11px] font-semibold tracking-widest uppercase text-[var(--faint)] mb-2">Tensions</p>
        ...
        {blobId && (
          <div className="mt-6 pt-4 border-t border-[var(--line)] flex flex-wrap items-center gap-x-3 gap-y-2 text-xs">
            <span className="inline-flex items-center gap-1.5 font-semibold text-[var(--ink)]">
              <WalToken size={15} variant="color" />
              Stored on Walrus
            </span>
            <span className="font-mono text-[var(--faint)]">{blobId.slice(0, 14)}…</span>
            ...
            {sessionId && <span className="text-[var(--faint)] font-mono">· {sessionId.slice(0, 8)}</span>}
            {stored && <span className="text-[var(--faint)]">· {stored}</span>}
          </div>
        )}
      </div>
    </div>
